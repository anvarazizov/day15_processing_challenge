...
void setup()
{
  size(800, 600, P3D); //screen proportions
  noStroke();
  minim = new Minim(this);
  //  player = minim.getLineIn(Minim.STEREO, 4096);
  player = minim.loadFile("song.mp3", 4096);
...
class Waveform
{
  ...
  void plot()
  {
    for (int i = 0; i < fftLog.avgSize(); i++)
    {
      ...
      x = i*w;
      y = frameCount*5;
      z = height/4-fftLog.getAvg(i)*1; //change multiplier to reduces height default '10'
...
void keyPressed()
{
  ...
  if (mm==null) {
    if (key=='s' || key=='S') {
      ...
      sketchPath("movie.mov"), width, height, 25);
